...
- **Multi-root aware.** Every workspace folder that is a git repo and has a `.gitlab-ci.yml` is watched. Pipelines are grouped per repository:
  ...
  📦 repo-b · dev (20)
     🏃  456 · running · dev
  ```
- **Two homes, kept in sync.** The tree shows up under **Source Control** *and* the **Explorer** — pick whichever panel you live in. Clicking a repository in one panel expands it in the other, so the project you pick opens in **Pipelines** too.

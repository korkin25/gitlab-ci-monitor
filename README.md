...
- **Actions.** Inline buttons and a right-click menu. On a **pipeline** — **Stop** (running) or **Run new pipeline** (finished) inline; the menu also has **Open commit in GitLab**. On a **job** — **Open job log** (live) and **Stop** (running) inline; the menu adds retry (finished), play (manual), and open in GitLab.

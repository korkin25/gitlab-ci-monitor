...
- **Actions.** Right-click for the context menu: on a **pipeline** — **retry** its failed jobs (only offered on a failed/canceled pipeline), **run a new pipeline** on its ref, cancel, or **open the commit** it ran against; on a **job** — retry (finished), cancel (running), or play (manual), plus open the job in GitLab. Inline buttons: retry on failed pipelines, cancel on running, run on finished. (Note: "Retry pipeline" re-runs a pipeline's *failed* jobs — for a fresh run use "Run new pipeline".)

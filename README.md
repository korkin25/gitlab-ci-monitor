...
- **Actions.** Right-click for the context menu: on a **pipeline** — retry, cancel, or **open the commit** it ran against; on a **job** — retry (finished), cancel (running), or play (manual), plus open the job in GitLab. Pipelines also keep inline retry/cancel buttons.
